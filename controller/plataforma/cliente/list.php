...
try {
    ...
    $stmt = $oConexao->prepare(
        'SELECT
			cl.id as idcliente,ce.id,cl.nome,cl.cpf,cl.email,cl.telefonecelular,cl.datalogin
		FROM cliente_estabelecimento ce
		INNER JOIN cliente cl ON (ce.idcliente = cl.id)
		WHERE ce.idestabelecimento =:idestabelecimento AND ce.status = :status '.$search.'
		ORDER BY cl.id DESC
		LIMIT :offset,:limit'
    );
