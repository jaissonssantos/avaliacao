	...
	.factory('loginService', [ '$http', '$location', 'sessionSrv', '$timeout' , function( $http, $location, sessionSrv, $timeout ){
		...
		return {
			...
			login: function( user, scope ){
				...
				var $promise = $http.post('/controller/plataforma/plataforma/login', {email: user.email, password: user.password});
				$promise.then(function(item){
					...
					if(response.data.error){
						...
						scope.user.feedback = response.data.error;
			...
			logout: function(){
				...
				var $promise = $http.post('/controller/plataforma/plataforma/logout');
				$promise.then(function(item){
					if( item.data.status == 'success' ){
						$location.path('/login');
					}else if( item.data.status == 'error' ){
			...
			islogged: function(){
 				var $promise = $http.post('/controller/plataforma/plataforma/checksessionlogin');
 				return $promise;
			},
			...
			roles: function(){
				var $promise = $http.post('/controller/plataforma/plataforma/rolespermission');
				return $promise;
			}
